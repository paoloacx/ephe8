/*
 * main.js (v2.67 - Forcing Cache Refresh)
...
async function checkAndRunApp() {
    console.log("Iniciando Ephemerides v2.67 (Forcing Cache Refresh)..."); // Versión actualizada

    try {
        ui.setLoading("Iniciando...", true); // <- Error ocurre aquí si ui no está bien cargado
        initFirebase(); //
        ...
        // Asegurarse de que 'ui' y 'ui.setLoading' existan antes de llamarlo en el catch
        if (ui && typeof ui.setLoading === 'function') {
            ...
            // Fallback si ui.setLoading no está disponible ni siquiera aquí
            alert(`Error crítico durante el arranque: ${err.message}. Por favor, recarga.`);
        }
    }
}

// ... (Resto de main.js SIN CAMBIOS, igual que la v2.65 que te di antes) ...
...
async function handleFooterAction(action) {
    ...
    const userId = state.currentUser?.uid;
    switch (action) {
        case 'add': ui.openEditModal(null, []); break;
        case 'store': ui.openStoreModal(); break;
        ...
        case 'search':
            ...
            try {
                ...
                if (results.length === 0) {
                    ...
                    results.forEach(mem => { if (!mem.Nombre_Dia) { mem.Nombre_Dia = state.allDaysData.find(d => d.id === mem.diaId)?.Nombre_Dia || "Día"; } });
...
async function handleSaveDayName(diaId, newName, statusElementId = 'save-status') {
    if (!state.currentUser || !state.currentUser.uid) { ui.showModalStatus(statusElementId, `Debes estar logueado`, true); return; }
    const userId = state.currentUser.uid;
...
async function handleSaveMemorySubmit(diaId, memoryData, isEditing) {
    if (!state.currentUser || !state.currentUser.uid) { ui.showModalStatus('memoria-status', `Debes estar logueado`, true); return; }
    const userId = state.currentUser.uid;
    ...
    try {
        ...
        if (saveBtn) { saveBtn.disabled = false; saveBtn.textContent = isEditing ? 'Actualizar Memoria' : 'Añadir Memoria'; }
    } finally {
         if (saveBtn && saveBtn.disabled) { saveBtn.disabled = false; saveBtn.textContent = isEditing ? 'Actualizar Memoria' : 'Añadir Memoria'; }
    }
}
async function handleDeleteMemory(diaId, mem) {
    if (!state.currentUser || !state.currentUser.uid) { ui.showModalStatus('memoria-status', `Debes estar logueado`, true); return; }
    const userId = state.currentUser.uid;
    if (!mem || !mem.id) { ui.showModalStatus('memoria-status', `Error: Información de memoria inválida.`, true); return; }
    ...
    if (!confirmed) return;
    try {
        // Lógica de borrar Imagen eliminada
        await deleteMemory(userId, diaId, mem.id, null); // Pasar null para imagenURL
...
async function handleStoreCategoryClick(type) {
    ...
    try {
        let result;
        if (type === 'Nombres') { result = await getNamedDays(userId, 10); }
        else { result = await getMemoriesByType(userId, type, 10); }
        result.items.forEach(item => { if (!item.Nombre_Dia && item.diaId) { item.Nombre_Dia = state.allDaysData.find(d => d.id === item.diaId)?.Nombre_Dia || "Día"; } });
        ...
        } else { ui.showAlert(`Error al cargar ${type}: ${err.message}`); }
...
async function handleStoreLoadMore() {
    ...
    try {
        let result;
        if (currentType === 'Nombres') { result = await getNamedDays(userId, 10, lastVisible); }
        else { result = await getMemoriesByType(userId, currentType, 10, lastVisible); }
        result.items.forEach(item => { if (!item.Nombre_Dia && item.diaId) { item.Nombre_Dia = state.allDaysData.find(d => d.id === item.diaId)?.Nombre_Dia || "Día"; } });
        state.store.lastVisible = result.lastVisible;
        state.store.isLoading = false;
        ...
        if(loadMoreBtn) {
             ...
             setTimeout(() => { if (loadMoreBtn.textContent === "Error al cargar") { loadMoreBtn.disabled = false; loadMoreBtn.textContent = "Cargar más"; } }, 3000);
        }
...
function handleStoreItemClick(diaId) {
    ...
    if (!dia) { console.error("No se encontró el día:", diaId); return; }
    ui.closeStoreListModal();
    ui.closeStoreModal();
    const monthIndex = parseInt(dia.id.substring(0, 2), 10) - 1;
    if (state.currentMonthIndex !== monthIndex) { state.currentMonthIndex = monthIndex; drawCurrentMonth(); }
    setTimeout(() => { handleDayClick(dia); }, 100);
    window.scrollTo(0, 0);
}
function handleCrumbieClick() {
     if (!state.currentUser) { ui.showAlert("Debes iniciar sesión para usar Crumbie."); return; }
    const messages = ["¡Hola! ¿Qué buscamos?", "Pregúntame sobre tus recuerdos...", "¿Cuál es tu canción favorita?", "Buscando un día especial..."];
