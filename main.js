/*
 * main.js (v2.69 - Cache Bust Fix)
 * Controlador principal de Ephemerides.
 */

// --- Importaciones de Módulos (Con versión para cache bust) ---
import { initFirebase, db, auth } from './firebase.js?v=2.69';
import { initAuthListener, handleLogin, handleLogout, checkAuthState } from './auth.js?v=2.69';
import {
...
import { searchMusic, searchNominatim } from './api.js?v=2.69';
import { ui } from './ui.js?v=2.69';
import { showSettings } from './settings.js?v=2.69';
...
async function checkAndRunApp() {
    console.log("Iniciando Ephemerides v2.69 (Cache Bust Fix)...");
    ...
    try {
        ...
        const errorMsg = err.code === 'permission-denied'
            ...
            : `Error crítico: ${err.message}. Por favor, recarga.`;
        ui.setLoading(errorMsg, true);
    }
}
...
async function initializeUserSession() {
    ...
    try {
        ...
        if (state.allDaysData.length === 0) {
            console.error(`Error: No se cargaron días para ${userId}.`);
...
async function loadTodaySpotlight() {
    ...
    try {
        ...
        ui.updateSpotlight(dateString, "Error", []);
    }
}
...
async function handleEditFromPreview() {
    ...
    if (!dia || !state.currentUser?.uid) {
        console.error("No hay día o usuario para editar desde preview.");
        if (!state.currentUser) ui.showAlert("Debes iniciar sesión para poder editar.");
        return;
    }
    const userId = state.currentUser.uid;
    ui.closePreviewModal();
...
async function handleSaveDayName(diaId, newName, statusElementId = 'save-status') {
    ...
    const userId = state.currentUser.uid;
    const finalName = newName?.trim() || "Unnamed Day";
    try {
        ...
        drawCurrentMonth();
        if (statusElementId === 'save-status' && state.dayInPreview?.id === diaId) {
             const dia = state.dayInPreview;
             const dayNameUI = finalName !== 'Unnamed Day' ? ` (${finalName})` : '';
             const editModalTitle = document.getElementById('edit-modal-title');
             if (editModalTitle) editModalTitle.textContent = `${dia.Nombre_Dia}${dayNameUI}`;
...
async function handlePlaceSearch(term) {
    if (!term?.trim()) return;
    try {
        const results = await searchNominatim(term.trim());
...
async function handleStoreCategoryClick(type) {
    ...
    try {
        const result = (type === 'Nombres')
            ? await getNamedDays(userId, 10)
            : await getMemoriesByType(userId, type, 10);
        ...
        const errorMsg = (err.code === 'failed-precondition' || err.message.includes("index"))
            ? "Error de Firebase: Se requiere un índice. Revisa la consola (F12) para ver el enlace de creación."
            : `Error al cargar ${type}: ${err.message}`;
...
async function handleStoreLoadMore() {
    ...
    if (isLoading || !currentType || !state.currentUser?.uid) return;
    const userId = state.currentUser.uid;
    ...
    try {
        const result = (currentType === 'Nombres')
            ? await getNamedDays(userId, 10, lastVisible)
            : await getMemoriesByType(userId, currentType, 10, lastVisible);
        result.items.forEach(item => {
             if (!item.Nombre_Dia && item.diaId) {
                item.Nombre_Dia = state.allDaysData.find(d => d.id === item.diaId)?.Nombre_Dia || "Día";
            }
